...
public class BookService {
    ...
    // MySQL-safe per-user counter: try UPDATE, if 0 rows then INSERT, handle race with duplicate key.
    @Transactional
    ...
    // NEW: fetch by per-user number (userBookNo) used by /api/books/no/{no}
    @Transactional(readOnly = true)
