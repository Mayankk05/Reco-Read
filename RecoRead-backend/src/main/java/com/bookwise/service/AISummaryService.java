...
public class AISummaryService {
    ...
    private static final int RETRY_INPUT_CHARS = 2000;
    ...
    public SummaryResponse generateSummary(Long bookId, Long userId, SummaryRequest request) {
        ...
        if (originalText.length() > MAX_INPUT_CHARS) {
            throw new ExternalApiException("Input text too long. Please keep under " + MAX_INPUT_CHARS + " characters.");
        }
        ...
        try {
            ...
            String summaryText = callGeminiWithFallback(prompt, originalText);
    ...
    private String buildPrompt(String originalText) {
        return "Summarize the following passage in 2-3 clear sentences for a general reader:\n\n" + originalText;
    }
    ...
    private String callGeminiWithFallback(String prompt, String originalText) {
        String summary = callGeminiWithRetry(prompt, 3);
        if (isUnavailableSummary(summary)) {
            log.warn("Gemini returned unavailable summary for full input, retrying with smaller chunk...");
            String truncated = originalText.length() > RETRY_INPUT_CHARS
                    ? originalText.substring(0, RETRY_INPUT_CHARS)
                    : originalText;
            String prompt2 = buildPrompt(truncated);
            summary = callGeminiWithRetry(prompt2, 2);
        }
        if (isUnavailableSummary(summary)) {
    ...
    private boolean isUnavailableSummary(String summary) {
        return summary == null || summary.isBlank() ||
                summary.contains("[Summary unavailable");
    }
    ...
    private String callGemini(String prompt) {
        ...
        try {
            ...
            log.debug("Gemini API response: {}", resp);
    ...
    public void deleteSummary(Long summaryId, Long userId) {
        var summary = bookSummaryRepository.findById(summaryId)
                .orElseThrow(() -> new ExternalApiException("Summary not found."));
        if (!summary.getBook().getUser().getId().equals(userId)) {
            throw new ExternalApiException("You do not have permission to delete this summary.");
        }
        bookSummaryRepository.delete(summary);
    }
